...
class train_dataset(Dataset):
    ...
    def __init__(self, train_data, img_size):
...
class Label_loader:
    def __init__(self, video_folders, data_root, dataset, test_data):
        assert dataset in ('ped2', 'avenue', 'shanghaitech'), f'Did not find the related gt for \'{dataset}\'.'
        self.name = dataset
        self.frame_path = test_data
        sefl.data_root = data_root
    ...
    def load_shanghaitech(self):
        np_list = glob.glob(f'{self.data_root + self.name}/frame_masks/')
        np_list.sort()
